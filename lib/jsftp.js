...
/**
 * Lists a folder using mlsd contents using a passive connection.
 *
 * @param {String} path Remote path for the file/folder to retrieve
 * @param {Function} callback Function to call with errors or results
 */
Ftp.prototype.mlsd = function(path, callback) {
  if (arguments.length === 1) {
    callback = arguments[0];
    path = '';
  }
  ...
  self.getPasvSocket(function(err, socket) {
    ...
    socket.once('close', function(err) {
        ...
        for(var i=0; i<data.length; i++) {
            ...
            // Check if file
            if(line[0] == 'type=file') {
                // Check if it is not a temporary file
                if(line[7].indexOf('.pureftpd-upload') == -1) {
                    var date = line[2].replace('modify=', '');
                    date = date.substr(0, 8) + 'T' + date.substr(8);
